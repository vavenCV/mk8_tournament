use crate::db::{
    model::Player,
    schema::{CreatePlayerSchema, FilterOptions, UpdateNoteSchema},
    AppState,
};
...
pub async fn player_list_handler(
    ...
    if query_result.is_err() {
        let message = "Something bad happened while fetching all player items";
        return HttpResponse::InternalServerError()
            .json(json!({"status": "error","message": message}));
    }

    let players = query_result.unwrap();
...
async fn create_player_handler(
    body: web::Json<CreatePlayerSchema>,
    data: web::Data<AppState>,
) -> impl Responder {
    let query_result = sqlx::query_as!(
        PlayerModel,
    ...
    match query_result {
        ...
        Err(e) => {
                ...
                .contains("duplicate key value violates unique constraint")
            {
                ...
                .json(serde_json::json!({"status": "fail","message": "Player with that title already exists"}));
            }
